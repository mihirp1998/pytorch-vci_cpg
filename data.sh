...
indir=/Users/mihir/Documents/projects/pytorch-vcii_cpg/data/train
...
do
	...
	if [ $group_idx = "1" ]; then
		...
		if [ $mode = "4digits" ]; then
			last_frame=${indir}/${video_name}$(printf "%04d" ${last_idx}).jpg
		fi
		...
		# 		fi
		# 		if [ $mode = "4digits" ]; then
		# 			cur_frame=${indir}/${video_name}$(printf "%04d" ${cur_idx}).jpg
		# 			prev_frame=${indir}/${video_name}$(printf "%04d" ${prev_idx}).jpg
		# 			next_frame=${indir}/${video_name}$(printf "%04d" ${next_idx}).jpg
		# 		fi
